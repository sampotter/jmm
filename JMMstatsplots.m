...
method = ["JMM1dijkstra_slo","JMM2dijkstra_slo","JMM3dijkstra_slo",...
            "FMM_slo","olim8mp0_slo"];
metname = ["JMM1-Dijkstra","JMM2-Dijkstra","JMM3-Dijkstra","FMM","OLIM8mp0"];
slo = ['1','p','v','m','g'];
...
for itable = 1 : Ntable    
    fprintf('\n\n');
    fprintf(strcat('Method &',metname(1),'&',metname(2),'&',metname(3),'&',metname(4),'&',metname(5),'\\'));
    fprintf('\n');
    for islo = 1 : 5
        fprintf(strcat(sloname(islo),'&'));
        fprintf('%.2f & %.2f & %.2f & %.2f & %.2f \\ \n',p(islo,1,itable),...
            p(islo,2,itable),p(islo,3,itable),p(islo,4,itable),p(islo,5,itable));
    end
end
end
%%
