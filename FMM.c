...
//--------- TWO-PT-UPDATE ----------------
struct mysol two_pt_update(double u0,double u1,double s);
...
void marcher(void) {
  int m,m1,ind,ind0,ind1,i0,j0,p0,p1,p1mp0;
  double utemp;
  struct mysol sol; 
    		...
    		if( status[ind] < 2 ) { // ind is not accepted
    			...
    			for( m1 = 0; m1 < 2; m1++ ) {
					...
					if( ch1 == 'y' ) { // can do two-pt-update with ind0 and ind1
						...
						p1mp0 = p1 - p0;
// 						v = find_dir01(p1mp0);
						sol = two_pt_update(u[p0],u[p1],slo[ind]);
						if( sol.ch == 'y' ) {
							ch_1ptu = 'n';
							if( sol.u < u[ind] ) {
								u[ind] = sol.u;
...
int main() {
	...
	sprintf(fname,"Data/FMM_ibox_slo%c.txt",slo_fun);
		...
		for( j = 0; j < NY; j++ ) {
			for( i = 0; i < NX; i++ ) {
				...
				if( status[ind] == 0 ) u[ind] = INFTY;
   			     ...
   			     if( print_errors == 'y' )  fprintf(ferr,"%.4e\t",dd);
			}
  		    if( print_errors == 'y' ) fprintf(ferr,"\n");
		}
		if( print_errors == 'y' ) fclose(ferr);
		printf("slo_fun = %c: # accepted = %i, umax = %.4e,  NX = %i, NY = %i, errmax = %.4e, (imax = %i), erms = %.4e\n",
			  slo_fun,m,umax,NX,NY,errmax,imax,sqrt(erms/m));
		fprintf(fstats,"%i\t%.4e\t%.4e\t%.4e\t%.4e\t%g\n",
			NX,errmax,sqrt(erms/m),errmax/umax,sqrt(erms/urms),cpu);
