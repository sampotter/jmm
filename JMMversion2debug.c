...
void dijkstra_init() {
  ...
  struct myvector z;
    ...
    jmin = max(0,j0 - KY);
    jmax = min(ny1,j0 + KY);
//  	printf("Initial box: imin = %i, imax = %i, jmin = %i, jmax = %i\n",imin,imax, jmin,jmax);
...
//     		printf("ind = %i, u = %.4e, gu = (%.4e,%.4e)\n",ind,u[ind],gu[ind].x,gu[ind].y);
    		if( i == imin || i == imax || j == jmin || j == jmax ) {
//     			printf("Boundary point: i = %i, j = %i\n",i,j);
    			status[ind] = 1;
    			addtree(ind,count,tree,pos,u);
//     			printf("Return from addtree: count = %i\n",*count);
    		}
    	}
    } 
//     printf("dijkstra_init: count = %i\n",*count);          
}
...
int dijkstra_main_body() {
	...
	double *par1,*par2; // # of parameters for nonlinear equation for 1ptu and 2ptu
  	int npar1 = 17, npar2 = 41, ncpar = 4; // # of parameters for nonlinear equation for 1ptu and 2ptu
    struct mysol sol; 
...
// 	printf("in dijkstra_main_body: count = %i\n",*count);
...
// 	printf("bcount = %i, test: %i \n",bcount,2*(imax-imin+jmax-jmin));	
// 		for( i = 0; i < bcount; i++ ) {
// 			printf("bdry[%i] = %i, u[%i] = %.4e\n",i,bdry[i],bdry[i],blist[i]);
// 		}
...
// 		set up buckets
	...
	jbdry = 0;
	ind = bdry[jbdry];
// 		printf("j = %i, ind = %i, u = %.4e,iskip = %i, Bmax = %.4e\n",j,ind,u[ind],iskip,Bmax);
	ibcurrent = adjust_bucket(ind,u[ind],0.0,gap,bucket,list);
	jbdry = 1;
	while( jbdry < bcount && blist[jbdry] < Bmax ) {
		ind = bdry[jbdry];
// 			printf("j = %i, ind = %i, u = %.4e, list[%i].ibucket = %i\n",j,ind,u[ind],ind,list[ind].ibucket);
...
// 		printf("jbdry = %i, bcount = %i, vcurrent = %.4e\n",jbdry,bcount,vcurrent);
		// add points to buckets to the boundary
		if( jbdry < bcount ) {
			Bmax = vcurrent + gap*Nb1;
			while( jbdry < bcount && blist[jbdry] < Bmax ) {
				ind = bdry[jbdry];
// 				printf("jbdry = %i, ind = %i, u = %.4e, list[%i].ibucket = %i,minval = %.4e, Bmax = %.4e\n",jbdry,ind,u[ind],ind,list[ind].ibucket,minval,Bmax);
				knew = adjust_bucket(ind,u[ind],minval,gap,bucket,list);
				jbdry++;
			}		
		}		
...
int adjust_bucket(int ind,double newval,double v,double g,struct mybucket *bucket,struct mylist *list) {
	...
	knew = k%Nbuckets;
// 	printf("ind = %i,u = %.4e,knew = %i,list[%i].ibucket = %i\n",ind,newval,knew,ind,list[ind].ibucket);
