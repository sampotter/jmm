...
** Things to do
   ...
   4. [ ] compute ground truth amplitude
      1. [ ] geometric factor
      2. [ ] + UTD
   5. [X] add postprocessing pass to compute geometric amplitude
      - [X] try doing it in terms of amplitude
      - [X] try doing it in terms of intensity (*probably better*)
   6. [ ] add UTD (may need t_in, t_out)
   7. [ ] compute u by patching fields
   8. [-] 3D slice plots
   ...
   9. [ ] measure time and output it
   10. [ ] make error plots
   11. [ ] make timing plots
   12. [ ] fix O(N^2) runtime (data structure problem)
   13. [ ] simplify paper to c = 1 w/o extra c varying stuff
   14. [ ] build VTK with mathtex support (see [[https://github.com/pyvista/pyvista/issues/83][this issue]])
   15. [ ] finish writing
   16. [ ] send to ramani & masha
   17. [ ] make nice 3D figures using inkscape
