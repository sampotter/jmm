...
// segments of rays are approximated with cubic curves with
// slope a0 determined by Hermite interpolant and slope a1 free

// Compile command: gcc -Wall slowness_and_uexact.c Newton.c dijkstra2Dfreea1.c -lm -O3
...
// 		double *J,double *llim,double *ulim,double *par,int dim) {
...
int main() {
	...
	NWTarg = (double *)malloc(2*sizeof(double));
	NWTres = (double *)malloc(2*sizeof(double));
	NWTdir = (double *)malloc(2*sizeof(double));
	NWTllim = (double *)malloc(2*sizeof(double));
	NWTulim = (double *)malloc(2*sizeof(double));
	NWTJac = (double *)malloc(4*sizeof(double));
	...
	sprintf(fname,"Data/dijkstra_freea1_ibox_slo%c.txt",slo_fun);
	fg = fopen(fname,"w");
